...
// Note for TMP102 thermometer only:
// Used to read the contents of the pointer register
//uint32_t I2C_Recv6(int8_t slave){
void I2C_Recv6(int8_t slave, uint8_t data[]){
  ...
  do{
    ...
    while(I2C0_MCS_R&I2C_MCS_BUSY){};				// wait for transmission done
		...
		// TODO: call nunchuck_decode_byte to remove XOR encryption
    //data1 = (I2C0_MDR_R&0xFF);       				// MSB data sent first
	  data[0] = nunchuk_decode_byte(I2C0_MDR_R&0xFF);       				// MSB data sent first
    I2C0_MCS_R = (0
                         | I2C_MCS_DATACK      // positive data ack
                       //  | I2C_MCS_STOP     // generate stop
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C0_MCS_R&I2C_MCS_BUSY){};				// wait for transmission done
    //data2 = (I2C0_MDR_R&0xFF);       				// LSB data sent last
		data[1] = nunchuk_decode_byte(I2C0_MDR_R&0xFF);       				
    I2C0_MCS_R = (0
                         | I2C_MCS_DATACK      // positive data ack
                       //  | I2C_MCS_STOP     // generate stop
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C0_MCS_R&I2C_MCS_BUSY){};				// wait for transmission done
    //data3 = (I2C0_MDR_R&0xFF);       				// LSB data sent last
		data[2] = nunchuk_decode_byte(I2C0_MDR_R&0xFF);
    I2C0_MCS_R = (0
                         | I2C_MCS_DATACK      // positive data ack
                       //  | I2C_MCS_STOP     // generate stop
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C0_MCS_R&I2C_MCS_BUSY){};				// wait for transmission done
    //data4 = (I2C0_MDR_R&0xFF);       				// LSB data sent last
		data[3] = nunchuk_decode_byte(I2C0_MDR_R&0xFF);
    I2C0_MCS_R = (0
                         | I2C_MCS_DATACK      // positive data ack
                       //  | I2C_MCS_STOP     // generate stop
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C0_MCS_R&I2C_MCS_BUSY){};				// wait for transmission done
    //data5 = (I2C0_MDR_R&0xFF);       				// LSB data sent last
		data[4] = nunchuk_decode_byte(I2C0_MDR_R&0xFF);
    I2C0_MCS_R = (0
                       //  & ~I2C_MCS_ACK     // negative data ack (last byte)
                         | I2C_MCS_STOP     // generate stop
                       //  & ~I2C_MCS_START   // no start/restart
                         | I2C_MCS_RUN);    // master enable
    while(I2C0_MCS_R&I2C_MCS_BUSY){};				// wait for transmission done
    //data6 = (I2C0_MDR_R&0xFF);       				// LSB data sent last
		data[5] = nunchuk_decode_byte(I2C0_MDR_R&0xFF);
    retryCounter = retryCounter + 1;        // increment retry counter
  }                                         // repeat if error
  while(((I2C0_MCS_R&(I2C_MCS_ADRACK|I2C_MCS_ERROR)) != 0) && (retryCounter <= MAXRETRIES));
	// return nothing
