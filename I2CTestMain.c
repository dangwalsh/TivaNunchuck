...
void displayVal(uint8_t * val) { 
	char str[8];
	int index = 0;
	// next character is hundreds digit if not zero
  if(((*val)/100)%10 != 0){
    str[index] = (((*val)/100)%10) + '0';
    index = index + 1;
  // hundreds digit is not zero so next character is tens digit
    str[index] = (((*val)/10)%10) + '0';
    index = index + 1;
  }
	// hundreds digit is zero so next character is tens digit only if not zero
  else if(((*val)/10)%10 != 0){
...
void convert(uint8_t * data) {
	uint8_t joy_x_axis = data[0];
  uint8_t joy_y_axis = data[1];
  uint8_t accel_x_axis = (data[2] << 2);
  uint8_t accel_y_axis = (data[3] << 2);
  uint8_t accel_z_axis = (data[4] << 2);
  uint8_t z_button = 0;
  uint8_t c_button = 0;
	
	displayVal(&joy_x_axis);
	displayVal(&joy_y_axis);
  // byte data[5] contains bits for z and c buttons
  // it also contains the least significant bits for the accelerometer data
  // so we have to check each bit of byte data[5]
  if ((data[5] >> 0) & 1){
		z_button = 1;
	} 
	displayVal(&z_button);
...
//done testing
  ...
  while(1){
    ...
    rawData = I2C_Recv2(0x48);            // temperature data stored in 12 or 13 MSBs 
																					// (Figure 7.XX chapter7-10-3.ulb, reply 0x1660, 22.375 C)
